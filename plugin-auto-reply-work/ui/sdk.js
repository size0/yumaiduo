...
/**
 * The gateway may hand the iframe either its /ui mount or the plugin gateway
 * root. API calls always need to travel through the /ui mount, but callers
 * should still use the documented relative `api/...` form.
 */
export function normalizeGatewayRequestPath(gatewayBase, path) {
    const rel = String(path ?? '').replace(/^\/+/, '');
    if (!rel.startsWith('api/'))
        return rel;
    let pathname = '';
    try {
        pathname = new URL(String(gatewayBase), 'https://plugin.local').pathname;
    }
    catch {
        pathname = String(gatewayBase).split('?')[0];
    }
    return /\/ui\/?$/u.test(pathname) ? rel : `ui/${rel}`;
}
...
export function createPluginSdk(options = {}) {
    ...
    function ready() {
        ...
        readyPromise = new Promise((resolve, reject) => {
            ...
            timer = setTimeout(() => {
                ...
                readyPromise = null;
                pendingReadyReject = null;
                reject(new Error('插件握手超时：未收到核心 init 消息'));
            }, readyTimeoutMs);
    ...
    async function authedFetch(path, init = {}) {
        ...
        const url = buildGatewayUrl(session.gatewayBase, normalizeGatewayRequestPath(session.gatewayBase, path));
        const headers = new Headers(init.headers);
        headers.set('X-Plugin-Session', session.token);
