...
export function createOperatorApi({ config, platformRuntime, backendClient, storage, logger = console } = {}) {
  if (!backendClient || typeof backendClient.getRuntimeSettings !== 'function') throw new TypeError('operator API backendClient is required');
  if (typeof platformRuntime?.createClient !== 'function') throw new TypeError('platformRuntime.createClient is required');
  const requiredStores = ['eventStore', 'conversationContextStore', 'agentEvaluationStore', 'agentRunStore', 'agentManualTaskStore', 'agentHumanComparisonStore'];
  if (!storage || requiredStores.some((name) => !storage[name])) throw new TypeError('operator API storage dependencies are required');
  const { eventStore, conversationContextStore, agentEvaluationStore, agentRunStore, agentManualTaskStore, agentHumanComparisonStore } = storage;
