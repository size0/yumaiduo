...
function quoteSupersessionPreview(preview, quoteContext, envelope, settings = {}) {
  const facts = quoteContext?.facts ?? {};
  ...
  const replacesAreaQuote = firstImageUrl(envelope?.payload)
    ...
    && Number(facts.quote_expires_at ?? 0) > Date.now()
...
function autoQuoteReplyAction(envelope, preview, enabled, allowRecognitionFollowUp = false, settings = {}) {
  let text = String(preview?.reply_text ?? '').trim();
  // A quote failure is an authoritative deterministic safety outcome. Never
  // let a stale editable template weaken it or claim a human handoff that was
  ...
  if (configuredFailure) {
    const missing = Array.isArray(preview?.missing_fields) ? preview.missing_fields.filter((item) => typeof item === 'string' && item.trim()).join('、') : '';
  ...
  const action = autoReplyAction(envelope, text, origin, 'quote-reply');
  ...
  // Only the recognition reply sent by this exact event may be followed by its
  // quote. A concurrent inherited-image event must not bypass dedupe.
  return configuredAction && allowRecognitionFollowUp ? { ...configuredAction, allow_plugin_followup: true } : configuredAction;
}
...
function imageFailureReplyAction(envelope, mayFollowRecognition, enabled, settings = {}) {
  ...
  const action = mayFollowRecognition
    ? quoteFollowUpAction(envelope, text)
    : autoReplyAction(envelope, text, 'recognition_failure', 'recognition-failure-reply');
...
function duplicateQuoteClosureAction(envelope, quoteContext) {
  ...
  const total = positiveCents(facts.quote_total_cents);
  const count = positiveCents(facts.quote_ticket_count);
  const delivered = facts.quote_reply_delivered === true && Number(facts.quote_expires_at ?? 0) > Date.now();
  ...
  return autoReplyAction(envelope, reply, 'quote_follow_up', 'duplicate-quote-closure');
}

function deliveredUnitQuoteQuestionPreview(envelope, quoteContext) {
  const facts = quoteContext?.facts ?? {};
  const unit = positiveCents(facts.quote_unit_cents);
  if (!unit || Number(facts.quote_expires_at ?? 0) <= Date.now() || facts.quote_reply_delivered !== true
...
function completedUnitQuotePreview(envelope, quoteContext, settings = {}) {
  const facts = quoteContext?.facts ?? {};
  const unit = positiveCents(facts.quote_unit_cents);
  const expiresAt = Number(facts.quote_expires_at ?? 0);
  if (!unit || expiresAt <= Date.now() || positiveCents(facts.quote_total_cents) || positiveCents(facts.quote_ticket_count)
...
function hasActiveQuote(facts) {
  return Number(facts?.quote_expires_at ?? 0) > Date.now()
