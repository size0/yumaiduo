...
test('scanner captures a seller reply against the prior buyer turn and shadow plan', async () => {
  ...
  const scanner = createAgentHumanComparisonScanner({
    ...
    eventStore: {
      async list() { return [{ key: '107:event-1', envelope: { tenantId: '107', id: 'event-1', payload: { accountUnb: 'shop', chatId: 'chat', peerUnb: 'buyer', remoteMessageId: 'buyer-message-1', content: '两张多少钱', imageUrls: ['https://img/a.jpg'] } } }]; },
      async wasSentMessage(_tenant, _chat, messageId) { return messageId === 'plugin-message'; },
    },
    agentRunStore: { async list() { return [{ event_key: '107:event-1', mode: 'shadow', status: 'completed', result: { proposed_reply: '实时价格是50元。', trace: [{ intent: '核价', action: 'quote_realtime', goal: '获得权威报价' }] } }]; } },
...
test('scanner waits for a completed shadow result instead of recording an empty comparison', async () => {
  const captured = [];
  const scanner = createAgentHumanComparisonScanner({
    conversationContextStore: { async listRecent() { return [{ tenant_id: '107', account_unb: 'shop', chat_id: 'chat', peer_unb: 'buyer', facts: {}, messages: [] }]; } },
    eventStore: {
      async list() { return [{ key: '107:event-1', envelope: { tenantId: '107', id: 'event-1', payload: { remoteMessageId: 'buyer-message-1' } } }]; },
      async wasSentMessage() { return false; },
    },
    agentRunStore: { async list() { return [{ event_key: '107:event-1', mode: 'shadow', status: 'processing', result: null }]; } },
    coreFor() { return { im: { async listMessages() { return { items: [
      { direction: 'inbound', messageId: 'buyer-message-1', content: '你好', sentAt: '2026-08-22T04:00:01Z' },
      { direction: 'outbound', messageId: 'seller-message-1', content: '你好，请问需要什么场次？', sentAt: '2026-08-22T04:00:02Z' },
    ] }; } } }; },
    comparisonStore: { async capture(input) { captured.push(input); } },
  });

  assert.equal((await scanner.tick()).captured, 0);
