...
唯一交易规范见 `E:\鱼麦多\v4\V4_UNIFIED_AGENT_LIANGPIAO_SPEC.md`。固定交易骨架由规则状态机和后端安全门禁保证；Agent 可直接回复、追问、查询和报价，改价、付款后下单、出票和退款仍只能通过受控后端流程执行。
...
- 会话策略：管理客服人设、业务背景、知识补充、回复风格、记忆范围、Agent 阶段门禁和人工接管策略
- 知识库：从 `/api/settings/knowledge` 动态读取可编辑、可分类、可启停的常见问题条目；只有启用条目注入 Agent，价格、库存和订单事实仍以实时工具与后端状态为准
- 接口地址：可编辑
...
完成出票系统联调、沙箱/测试订单验证后，才允许在生产环境显式改为 `true`。为避免触发鱼麦多平台订单读取流控，订单页面不会后台定时轮询：操作员点击出票工作台的“待出票”菜单时读取一次，之后只通过“立即刷新/刷新订单”按钮主动读取。出票系统提交的电影、影院、场次、影厅、座位和取票码信息优先显示。接口返回的 `source` 固定为 `wanda`，不会同步良票订单。
