    ...
    def _seat_facts(
        ...
        for area in areas:
            ...
            for seat in raw_seats if isinstance(raw_seats, list) else []:
                ...
                member_status = seat.get("payMemberSeatStatus")
                activity_marks_wplus = (
                    isinstance(wplus_activity, Mapping)
                    and any(str(wplus_activity.get(key) or "").strip().upper() in {"W+", "WPLUS", "WPLUS会员"}
                            for key in ("type", "seatType", "seat_type", "channel", "name"))
                )
                physical_wplus = (
                    member_status in (1, "1", True, "true", "TRUE")
                    or "W+" in seat_area_name.upper()
                    or activity_marks_wplus
                )
