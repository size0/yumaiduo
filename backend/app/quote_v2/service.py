...
class QuoteV2Service:
    ...
    def persist(
        ...
        *,
        ...
        record = self._record_from_result(
            ...
            identity=identity, city_id=wanda_city_id, city_name=city_name, cinema_name=cinema_name,
    ...
    def _record_from_result(
        ...
        *,
        ...
        cost_values = [item["cost_fen"] for item in costs]
        unit_cost = cost_values[0] if cost_values and len(set(cost_values)) == 1 else None
        provider_total = (
            sum(cost_values) if pricing.request_type == "EXACT_SEATS" and cost_values
            else unit_cost * pricing.ticket_count
            if unit_cost is not None and pricing.ticket_count is not None else None
        )
        engine_result = pricing.calculation_evidence.get("result", {})
        ...
        return {
            ...
            "city": city_name or "", "cinema": _required(cinema_name, "cinema_name"),
            "cinema_address": str(cinema_address or "").strip() or None,
            ...
            "cost_fen": unit_cost,
            # Preserve the provider facts used by pricing so the quote
            # record/UI can distinguish cost, official price and sell price.
            "provider_amount_fen": provider_total,
            "member_unit_price_cents": unit_cost,
            "original_unit_price_cents": engine_result.get("original_unit_price_cents"),
            "base_unit_cents": engine_result.get("base_unit_cents"),
            "base_total_cents": engine_result.get("base_total_cents"),
...
class CanonicalQuoteRuntime:
    ...
    async def quote_structured(self, request: CanonicalQuoteRequest) -> dict[str, Any]:
        ...
        if request.ticket_count is not None and (
            type(request.ticket_count) is not int or not 1 <= request.ticket_count <= 20
        ):
        ...
        recognition = RecognitionResult(
            city_text=request.city, cinema_text=request.cinema,
            cinema_address=request.cinema_address, movie=request.movie,
    ...
    async def refresh_expired_auto_quote(
        ...
        seat_facts = await self._seats.resolve({
            ...
            "has_manual_mark": False if request_type == "EXACT_SEATS" else True,
        })
    ...
    def _render_buyer_reply(self, result: dict[str, Any], *, recognition: Any) -> dict[str, Any]:
        ...
        return {
            **result,
            ...
            # durable reply plan to RulesFirstRuntime.
    ...
    async def _quote_wanda(
        ...
        record = self._quotes.persist(
            ...
            cinema_address=route.wanda_cinema_address or getattr(recognition, "cinema_address", None),
            city_name=route.wanda_city_name or recognition.city_text,
