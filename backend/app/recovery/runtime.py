...
class RecoveryQuoteRuntime:
    ...
    async def process_image_event(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        current = recognition.model_dump(mode="json")
        context.merge_facts(current, stored=stored_facts)
        for key in ("city_text", "cinema_text", "movie", "show_date", "start_time", "hall", "dimension", "selected_seats"):
            if key in stored_facts and not getattr(recognition, key, None):
                mapping = {"city_text": "city", "cinema_text": "cinema", "show_date": "quote_date", "start_time": "showtime_start"}
                recognition = recognition.model_copy(update={key: stored_facts.get(mapping.get(key, key))})
        route_gate = await self.route.resolve_gate(recognition)
        if route_gate.status not in {"WANDA_SELF", "LIANGPIAO"}:
            return self._safe(route_gate)
        route = CinemaRouteResult.model_validate(route_gate.facts)
        if route.route != "WANDA_SELF":
            return self._safe(route_gate)
        show_gate = await self.show.resolve_gate({"route": route.route, "wanda_store_id": route.wanda_store_id,
        ...
        if not show_gate.success:
            return self._safe(show_gate)
        show = ShowResolutionResult.model_validate(show_gate.facts)
        seat_gate = await self.seat.resolve_gate({"route": route.route, "wanda_store_id": route.wanda_store_id,
            "wanda_show_id": show.wanda_show_id, "selected_seats": recognition.selected_seats,
            "has_manual_mark": False, "image_url": urls[0]})
        if not seat_gate.success:
            return self._safe(seat_gate)
        seat = SeatFactsResult.model_validate(seat_gate.facts)
        cost_gate = self.cost.resolve_cost_gate(show, seat)
        if not cost_gate.success:
            return self._safe(cost_gate)
        cost = WandaCostFacts.model_validate(cost_gate.facts)
        rules = self.rules_provider()
        pricing_gate = self.pricing.price_gate(cost, show, seat, rules)
        if not pricing_gate.success:
            return self._safe(pricing_gate)
        pricing = WandaPricingResult.model_validate(pricing_gate.facts)
        quote_gate = self.quotes.persist_gate(pricing, show, tenant_id=identity["tenant_id"], shop_id=identity["shop_id"],
            buyer_id=identity["buyer_id"], chat_id=identity["chat_id"], wanda_city_id=route.wanda_city_id or "",
            cinema_name=route.wanda_cinema_name or recognition.cinema_text or "", purchase_context_id=identity["purchase_context_id"],
            request_id=f'{identity["event_id"]}:recovery-quote', event_id=identity["event_id"],
            recognition_id=recognition.provider_recognize_id, message_id=identity.get("message_id"))
        result = {"status": "QUOTED" if quote_gate.success else quote_gate.status, "quote": quote_gate.facts.get("quote_record")}
        reply_gate = reply_eligibility_gate(result)
        return {**result, "reply_gate": reply_gate.model_dump(mode="json")}
