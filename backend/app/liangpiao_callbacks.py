...
class CallbackVerifier:
    ...
    def verify(self, raw_body: bytes, *, signature: str, timestamp: str, nonce: str,
        ...
        # Liangpiao historically used seconds, while newer callback deliveries
        # may use Unix milliseconds. Preserve the exact text for HMAC signing
        # and normalize only for freshness validation.
        timestamp_seconds = timestamp_value / 1000 if timestamp_value >= 10_000_000_000 else timestamp_value
        if abs(current - timestamp_seconds) > self._max_skew:
        ...
        expected = self.sign(raw_body, timestamp_text, nonce_value)
