...
class V4PricingEngine:
    ...
    def _price_wanda_seat(self, seat: PricingSeatFact, rules: PricingRulesSnapshot, is_vip: bool) -> int:
        ...
        # The sell price must cover both the provider/member cost and any
        # per-seat channel fee.  The old floor used only ``member`` and could
        # therefore produce a zero-margin or loss-making quote when a rule
        # selected zero adjustment.
        cost_floor = member + max(0, int(seat.channel_fee_cents or 0))
        lower = ((cost_floor + increment - 1) // increment) * increment
    ...
    def _dynamic_wanda_adjustment(original: int, member: int, bands: Sequence[WandaPricingBand]) -> int:
        ...
        for index, band in enumerate(bands):
            ...
            if Decimal(str(band.min_discount_percent)) <= ratio < maximum or (index == len(bands) - 1 and ratio <= maximum):
                if band.markup_percent is not None:
                    return int((Decimal(member) * Decimal(str(band.markup_percent)) / Decimal(100)).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
