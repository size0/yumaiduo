...
class PricingRulesSnapshot:
    ...
    def from_mapping(cls, source: Mapping[str, Any]) -> "PricingRulesSnapshot":
        ...
        wanda = tuple(WandaPricingBand(float(item["min_discount_percent"]), float(item["max_discount_percent"]), int(item["fixed_adjustment_cents"])) for item in payload.get("wanda_rules", []) if isinstance(item, Mapping))
