...
class RulesFirstDecisionEngine:
    ...
    def _record_quote(
        ...
        *,
        ...
        order_payload = order if isinstance(order, Mapping) else {}
        payload = envelope.get("payload") if isinstance(envelope.get("payload"), Mapping) else {}
        buyer_nick = _pick(
            order_payload, "buyerNick", "buyer_nick", "buyer_name", "userName", "user_name",
        ) or _pick(
            payload, "buyerNick", "buyer_nick", "buyer_name", "userName", "user_name",
        )
        record = {
    ...
    async def _reprice_after_cinema_choice(
        ...
        try:
            ...
            self._record_quote(identity, envelope, recognition, quote, source="buyer_cinema_choice", order=order)
        ...
        if isinstance(urls, list) and urls:
            ...
            if not self._ai_assist_enabled:
                ...
                try:
                    ...
                    self._record_quote(
                        ...
                        source="buyer_image", quote_error=quote_error, order=order,
                    )
            ...
            if explicit_seats and isinstance(durable_quote, Mapping):
                ...
                if seat_request is None:
                    ...
                    try:
                        ...
                        self._record_quote(
                            ...
                            source="buyer_seat_selection", order=order,
                        )
                ...
                try:
                    ...
                    self._record_quote(
                        identity, envelope, structured_request, quote, source="buyer_text", order=order,
                    )
                    reply = build_recognition_reply(
            ...
            if decision_reason in {
                ...
                try:
                    ...
                    self._record_quote(
                        identity, envelope, recognition, quote,
                        source="contextual_image", quote_error=quote_error, order=order,
                    )
                except Exception:
                    reply = render_template(
                        self._templates().recognition_failure_other_template,
