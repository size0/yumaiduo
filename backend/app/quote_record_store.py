...
class QuoteRecordStore:
    ...
    def save_quote(
        ...
        *,
        ...
        with self._lock:
            ...
            records.append(normalized)
            if predecessor is not None:
                predecessor_ids = {item.get("record_id") for item in lineage}
                for item in records:
                    if item.get("record_id") not in predecessor_ids:
                        continue
                    item.update({
                        "status": "superseded",
                        "quote_state": "SUPERSEDED",
    ...
    def mark_delivered(
        ...
        with self._lock:
            records = self._read_records()
            # Only the persisted record identity may change delivery state;
            # event_id and quote_id are related audit identities, not aliases.
            selected = next(
    ...
    def _terms_fingerprint(record: Mapping[str, Any]) -> str | None:
        ...
        city_identity = record.get("city") or (
            record.get("wanda_city_id") if record.get("provider_route") == "WANDA_SELF" else None
        )
        required = (
            city_identity, record.get("cinema"), record.get("movie"), date_value,
