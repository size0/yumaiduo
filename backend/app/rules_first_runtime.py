...
class RulesFirstRuntime:
    ...
    def accept_canonical_result(
        ...
        reply_items = [item for item in rendered_replies if isinstance(item, Mapping) and str(item.get("text") or "").strip()] if isinstance(rendered_replies, list) else []
        if not reply_items and rendered_text:
            reply_items = [{"kind": result.get("canonical_reply_kind") or "reply", "text": rendered_text}]
        actions: list[dict[str, object]] = [{
            ...
            "canonical_reply_kind": str(item.get("kind") or result.get("canonical_reply_kind") or ""),
            "dedupe_key": f"canonical-reply:{event_id}:{index}",
            "quote_record_id": (result.get("quote") or {}).get("record_id"),
        ...
        try:
            ...
            commands = self._store.complete_event(
                ...
                result={
                    ...
                    "canonical_quote_record_id": (result.get("quote") or {}).get("record_id"),
                    "canonical_purchase_context_id": (
                        (result.get("quote") or {}).get("purchase_context_id")
                        or (envelope.get("payload") or {}).get("itemId")
                    ),
                },
            )
            return {
        ...
        try:
            ...
            if result is None and self._canonical_text_handler is not None:
                ...
                if canonical is not None:
                    result = self._coordinator.record_event_decision(body, canonical)
            if result is None:
            ...
            if reduced.get("canonical_agent_status") == "AGENT_REPLY_UNAVAILABLE":
                self._create_manual_task_from_event(body, revision, str(decision.get("reason") or "canonical_agent_failed"))
            self._store.complete_event(
                ...
                result={
                    ...
                    "canonical_recognition": reduced.get("recognition"),
                    "canonical_quote_record_id": (reduced.get("quote") or {}).get("record_id"),
