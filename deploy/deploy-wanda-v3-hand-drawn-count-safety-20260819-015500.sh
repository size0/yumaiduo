...
expected_release=/opt/wanda-v3-backend/releases/v3-quote-safety-audit-20260819-014500
release=/opt/wanda-v3-backend/releases/v3-hand-drawn-count-safety-20260819-015500
canonical_dropin=/etc/systemd/system/wanda-v3-backend.service.d/10-v3-runtime.conf
source_dropin=/tmp/wanda-v3-hand-drawn-count-safety-20260819-015500.service.conf
backup=/tmp/wanda-v3-hand-drawn-count-safety-20260819-015500.previous.conf
...
from app.main import _recognition_needs_seat_or_count_confirmation
from app.schemas import Recognition
recognition = Recognition.model_validate({"image_type": "SEAT_MAP", "hand_drawn_circle": {"exists": True, "estimated_seat_count": 2}})
assert _recognition_needs_seat_or_count_confirmation(recognition, None) is True
PY
then
  rollback
  echo "hand-drawn count guard failed; rolled back" >&2
  exit 1
fi
