...
bo=/opt/wanda-v3-backend/releases/v3-wplus-exclusive-price-b-20260819-103500; po=/opt/wanda-preview-plugin/releases/v3-reply-template-completion-20260819-093000
bn=/opt/wanda-v3-backend/releases/v3-short-need-count-reply-20260819-104500; pn=/opt/wanda-preview-plugin/releases/v3-short-need-count-reply-20260819-104500
...
test ! -e "$bn"; test ! -e "$pn"; cp "$bc" /tmp/short-reply-b.conf; cp "$pc" /tmp/short-reply-p.conf
rollback(){ cp /tmp/short-reply-b.conf "$bc"; cp /tmp/short-reply-p.conf "$pc"; systemctl daemon-reload; systemctl restart "$b" "$p"; rm -rf "$bn" "$pn"; }
