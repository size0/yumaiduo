...
function optionalIntegrationUrl(env, name) {
  const source = String(env[name] ?? '').trim();
  if (!source) return null;
  let parsed;
  try { parsed = new URL(source); } catch { throw new ConfigurationError(`${name} must be a valid URL`); }
  const local = ['127.0.0.1', 'localhost', '::1'].includes(parsed.hostname);
  if (parsed.protocol !== 'https:' && !(parsed.protocol === 'http:' && local)) {
    throw new ConfigurationError(`${name} must use HTTPS or a loopback HTTP URL`);
  }
...
export function loadV2Config({ env, manifest }) {
  ...
  const orderEventUrl = optionalIntegrationUrl(env, 'ORDER_EVENT_OUTBOX_URL');
  const fulfillmentStatusUrl = optionalIntegrationUrl(env, 'ORDER_FULFILLMENT_STATUS_URL');
  const orderEventKey = optional(env, 'ORDER_EVENT_OUTBOX_KEY');
  const orderEventBridgeKey = optional(env, 'ORDER_EVENT_OUTBOX_BRIDGE_KEY');
  if (Boolean(orderEventUrl) !== Boolean(orderEventKey) || Boolean(orderEventUrl) !== Boolean(orderEventBridgeKey)) {
    throw new ConfigurationError('ORDER_EVENT_OUTBOX_URL, ORDER_EVENT_OUTBOX_KEY and ORDER_EVENT_OUTBOX_BRIDGE_KEY must be configured together');
  }
  if (fulfillmentStatusUrl && !orderEventUrl) throw new ConfigurationError('ORDER_FULFILLMENT_STATUS_URL requires ORDER_EVENT_OUTBOX_URL credentials');
  return Object.freeze({
    ...
    orderEventOutbox: Object.freeze({ url: orderEventUrl, statusUrl: fulfillmentStatusUrl, sharedSecret: orderEventKey, bridgeKey: orderEventBridgeKey }),
