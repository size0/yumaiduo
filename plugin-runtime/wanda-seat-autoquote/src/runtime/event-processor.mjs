...
const ORDER_EVENT_TYPES = new Set([
  'order.created', 'order.updated', 'order.paid', 'order.shipped', 'order.finished',
  'order.closed', 'order.refund.applied', 'order.refund.finished',
]);
...
function orderEventPayload(envelope, session, order) {
  const payload = objectPayload(envelope?.payload);
  const orderId = firstText(order?.order_id, payload.orderId, payload.order_id, payload.platformOrderId, payload.platform_order_id);
  // The delivery client intentionally signs only `payload`. Keep the
  // provider-neutral facts needed by the issuance queue in that signed
  // snapshot so the ticket system does not have to guess from message text.
  const orderSnapshot = order && typeof order === 'object' ? {
    order_id: firstText(order.order_id, orderId),
    tenant_id: firstText(order.tenant_id, envelope?.tenantId),
    shop_id: firstText(order.shop_id),
    buyer_id: firstText(order.buyer_id),
    chat_id: firstText(order.chat_id),
    order_status: text(order.order_status),
    paid_at: text(order.paid_at),
    amount_cents: Number.isSafeInteger(order.amount_cents) ? order.amount_cents : null,
    post_fee_cents: Number.isSafeInteger(order.post_fee_cents) ? order.post_fee_cents : null,
    refund_status: text(order.refund_status),
    quantity: Number.isSafeInteger(order.quantity) ? order.quantity : null,
    movie_name: firstText(order.movie_name, order.movieName, order.movie),
    cinema_name: firstText(order.cinema_name, order.cinemaName, order.cinema),
    showtime_start: firstText(order.showtime_start, order.showtimeStart, order.showtime),
    hall_name: firstText(order.hall_name, order.hallName, order.hall),
    seat_names: Array.isArray(order.seat_names) ? order.seat_names.filter((item) => text(item)).map((item) => text(item)) : null,
    cost_amount_fen: Number.isSafeInteger(order.cost_amount_fen) ? order.cost_amount_fen : null,
    quote_amount_fen: Number.isSafeInteger(order.quote_amount_fen) ? order.quote_amount_fen : null,
    quote_record_id: text(order.quote_record_id),
    binding_revision: Number.isSafeInteger(order.binding_revision) ? order.binding_revision : null,
  } : {};
  const sessionSnapshot = session && typeof session === 'object' ? {
    shop_id: firstText(session.accountUnb, session.account_unb),
    buyer_id: firstText(session.peerUnb, session.peer_unb),
    chat_id: firstText(session.chatId, session.chat_id),
  } : {};
  return {
    schema_version: 'wanda.order_event.v1',
    event_id: text(envelope?.id), tenant_id: text(envelope?.tenantId), source: 'wanda',
    source_order_id: orderId,
    event_type: text(envelope?.event), occurred_at_ms: Number(envelope?.ts),
    order_id: orderId,
    session: session && typeof session === 'object' ? {
      accountUnb: firstText(session.accountUnb, session.account_unb),
      peerUnb: firstText(session.peerUnb, session.peer_unb),
      chatId: firstText(session.chatId, session.chat_id),
    } : null,
    order: order && typeof order === 'object' ? structuredClone(order) : null,
    payload: {
      ...backendPayload(payload),
      ...Object.fromEntries(Object.entries(orderSnapshot).filter(([, value]) => value !== null && value !== '')),
      ...Object.fromEntries(Object.entries(sessionSnapshot).filter(([key, value]) => value !== null && value !== '' && !orderSnapshot[key])),
      ...(orderId ? { source_order_id: orderId } : {}),
    },
  };
}
...
export function createV2Runtime({ config, platform, backend, logger = console, store = new V2EventStore(join(config.dataDir, 'events.v2.json'), config.encryptionKey) }) {
  ...
  let orderEventPolling = false;
  let fulfillmentTimer = null;
  let fulfillmentPolling = false;
  let fulfillmentStatusTimer = null;
  let fulfillmentStatusPolling = false;
  ...
  async function start() {
    ...
    if (typeof backend.sendOrderEvent === 'function' && typeof store.claimOrderEvents === 'function') {
      orderEventTimer = setInterval(() => void pollOrderEvents(), 5_000);
      orderEventTimer.unref?.();
      void pollOrderEvents();
    }
    if (config.fulfillmentEnabled === true && typeof store.claimFulfillmentReceipts === 'function') {
      fulfillmentTimer = setInterval(() => void pollFulfillmentReceipts(), 5_000);
      fulfillmentTimer.unref?.();
      void pollFulfillmentReceipts();
    }
    if (typeof backend.sendFulfillmentStatus === 'function' && typeof store.claimFulfillmentStatuses === 'function') {
      fulfillmentStatusTimer = setInterval(() => void pollFulfillmentStatuses(), 5_000);
      fulfillmentStatusTimer.unref?.();
      void pollFulfillmentStatuses();
    }
  }
  async function stop() {
    stopped = true;
    ...
    if (orderEventTimer) clearInterval(orderEventTimer);
    if (fulfillmentTimer) clearInterval(fulfillmentTimer);
    if (fulfillmentStatusTimer) clearInterval(fulfillmentStatusTimer);
    retryTimer = null;
    reminderTimer = null;
    commandTimer = null;
    orderEventTimer = null;
    fulfillmentTimer = null;
    fulfillmentStatusTimer = null;
  ...
  async function enqueueFulfillmentStatus(value) {
    if (typeof store.enqueueFulfillmentStatus !== 'function') throw new Error('fulfillment_status_store_unavailable');
    if (typeof backend.sendFulfillmentStatus !== 'function') {
      return { created: false, deferred: true, reason: 'fulfillment_status_transport_unconfigured' };
    }
    const result = await store.enqueueFulfillmentStatus(value);
    if (typeof backend.sendFulfillmentStatus === 'function') void pollFulfillmentStatuses();
    return result;
  }
  ...
  async function pollOrderEvents() {
    if (stopped || orderEventPolling || typeof backend.sendOrderEvent !== 'function' || typeof store.claimOrderEvents !== 'function') return { processed: 0 };
    orderEventPolling = true;
    let processed = 0;
    try {
      const claimed = await store.claimOrderEvents({ limit: Math.max(1, Math.min(config.maxConcurrentRuns ?? 10, 20)) });
      for (const event of Array.isArray(claimed) ? claimed : []) {
        try {
          const response = await backend.sendOrderEvent(
            event.payload,
            `order-event:v1:${event.id}`,
          );
          await store.completeOrderEvent(event.id, event.lease, response);
          processed += 1;
        } catch (error) {
          logger.warn('order event delivery deferred', { eventId: event.event_id, error });
          await store.deferOrderEvent(
            event.id, event.lease, text(error?.message) ?? 'order_event_delivery_failed',
            { retryable: error?.retryable !== false },
          ).catch((deferError) => logger.warn('order event defer failed', { eventId: event.event_id, error: deferError }));
        }
      }
    } catch (error) {
      logger.warn('order event outbox poll deferred', { error });
    } finally {
      orderEventPolling = false;
    }
    return { processed };
  }

  async function pollFulfillmentStatuses() {
    if (stopped || fulfillmentStatusPolling || typeof backend.sendFulfillmentStatus !== 'function' || typeof store.claimFulfillmentStatuses !== 'function') return { processed: 0 };
    fulfillmentStatusPolling = true;
    let processed = 0;
    try {
      const claimed = await store.claimFulfillmentStatuses({ limit: Math.max(1, Math.min(config.maxConcurrentRuns ?? 5, 20)) });
      for (const event of Array.isArray(claimed) ? claimed : []) {
        try {
          const response = await backend.sendFulfillmentStatus(event.payload);
          await store.completeFulfillmentStatus(event.id, event.lease, response);
          processed += 1;
        } catch (error) {
          logger.warn('fulfillment status delivery deferred', { eventId: event.status_event_key, error });
          await store.deferFulfillmentStatus(event.id, event.lease, text(error?.message) ?? 'fulfillment_status_delivery_failed', {
            retryable: error?.retryable !== false,
          }).catch((deferError) => logger.warn('fulfillment status defer failed', { eventId: event.status_event_key, error: deferError }));
        }
      }
    } catch (error) {
      logger.warn('fulfillment status outbox poll deferred', { error });
    } finally {
      fulfillmentStatusPolling = false;
    }
    return { processed };
  }
  ...
  async function fulfillTenantOrder(tenantId, orderId, input, idempotencyKey) {
    ...
    async function reportFulfillmentStatus(record, phase, response = {}, error = '') {
      if (typeof backend.sendFulfillmentStatus !== 'function') return;
      try {
        await enqueueFulfillmentStatus({
          tenant_id: normalizedTenant,
          source: 'wanda',
          source_order_id: normalizedOrderId,
          status_event_key: `fulfillment:${normalizedTenant}:${normalizedOrderId}:${phase}`,
          status_version: ({ 'shipping-unknown': 10, 'shipping-succeeded': 20, 'message-unknown': 30, 'message-succeeded': 40 }[phase] ?? 20),
          shipping_status: record?.status === 'unknown' ? 'unknown' : 'succeeded',
          message_status: record?.message_status === 'sent' ? 'succeeded' : record?.message_status === 'unknown' ? 'unknown' : 'pending',
          response,
          error,
        });
      } catch (reportError) {
        logger.warn('fulfillment status enqueue deferred', { tenantId: normalizedTenant, orderId: normalizedOrderId, phase, error: reportError });
      }
    }
    ...
    if (existing?.status === 'submitted') {
      ...
      await reportFulfillmentStatus(repaired, 'message-succeeded', { repaired: true });
    ...
    try {
      ...
      const unknown = { ...baseRecord, status: 'unknown', failure_code: 'platform_ship_result_unknown', updated_at: new Date().toISOString() };
      await store.saveWandaFulfillment(unknown);
      await reportFulfillmentStatus(unknown, 'shipping-unknown', {}, unknown.failure_code);
    ...
    if (!shippedOrder(afterShip)) {
      const unknown = { ...baseRecord, status: 'unknown', failure_code: 'platform_ship_not_verified', updated_at: new Date().toISOString() };
      await store.saveWandaFulfillment(unknown);
      await reportFulfillmentStatus(unknown, 'shipping-unknown', {}, unknown.failure_code);
      throw fulfillmentError('platform_ship_not_verified', 502);
    }
    await reportFulfillmentStatus({ ...baseRecord, status: 'submitted', message_status: 'pending' }, 'shipping-succeeded', { verified: true });
    try { await deliverFulfillmentMessage(); }
    catch (error) {
      const unknown = { ...baseRecord, status: 'submitted', message_status: 'unknown', updated_at: new Date().toISOString() };
      await store.saveWandaFulfillment(unknown);
      await reportFulfillmentStatus(unknown, 'message-unknown', {}, text(error?.code ?? error?.message) ?? 'message_result_unknown');
      throw error?.code ? error : fulfillmentError('ticket_shipped_message_result_unknown', 503);
    }
    ...
    await reportFulfillmentStatus(completed, 'message-succeeded', { submitted: true });
    return { status: 'submitted', order: await client.orders.get(normalizedOrderId), fulfillment: completed };
  }

  async function acceptFulfillmentReceipt(value) {
    if (typeof store.enqueueFulfillmentReceipt !== 'function') throw fulfillmentError('fulfillment_receipt_store_unavailable', 503);
    if (value?.source && value.source !== 'wanda') throw fulfillmentError('fulfillment_receipt_source_invalid', 400);
    const result = await store.enqueueFulfillmentReceipt(value);
    void pollFulfillmentReceipts();
    return result;
  }

  function fulfillmentInputFromReceipt(receipt) {
    const source = receipt && typeof receipt === 'object' ? receipt : {};
    const codes = Array.isArray(source.ticket_codes)
      ? source.ticket_codes
      : typeof source.ticket_codes === 'string' ? source.ticket_codes.split(/[，,]/u).map((item) => item.trim()).filter(Boolean) : [];
    const rawSeats = source.seats ?? source.seat_names ?? [];
    const seats = Array.isArray(rawSeats)
      ? rawSeats
      : typeof rawSeats === 'string' ? rawSeats.split(/[，,]/u).map((item) => item.trim()).filter(Boolean) : [];
    const message = text(source.message_text ?? source.messageText)
      ?? [
        source.movie_name ?? source.movie,
        source.cinema_name ?? source.cinema,
        source.show_date ?? source.date,
        source.showtime_start ?? source.showtime,
        source.hall_name ?? source.hall,
        Array.isArray(seats) && seats.length ? `座位：${seats.join('、')}` : null,
        codes.length ? `取票码：${codes.join('、')}` : null,
      ].filter(Boolean).join('\n');
    return {
      ...source,
      city: source.city ?? source.city_name,
      movie_name: source.movie_name ?? source.movie,
      cinema_name: source.cinema_name ?? source.cinema,
      show_date: source.show_date ?? source.date,
      showtime_start: source.showtime_start ?? source.showtime ?? source.session,
      hall_name: source.hall_name ?? source.hall,
      seats,
      ticket_codes: codes.length ? codes : (typeof source.ticket_code === 'string' ? [source.ticket_code] : []),
      message_text: message,
    };
  }

  async function pollFulfillmentReceipts() {
    if (stopped || config.fulfillmentEnabled !== true || fulfillmentPolling || typeof store.claimFulfillmentReceipts !== 'function') return { processed: 0 };
    fulfillmentPolling = true;
    let processed = 0;
    try {
      const claimed = await store.claimFulfillmentReceipts({ limit: Math.max(1, Math.min(config.maxConcurrentRuns ?? 5, 10)) });
      for (const task of Array.isArray(claimed) ? claimed : []) {
        try {
          const result = await fulfillTenantOrder(task.tenant_id, task.source_order_id, fulfillmentInputFromReceipt(task.receipt), task.idempotency_key);
          await store.completeFulfillmentReceipt(task.id, task.lease, result);
          processed += 1;
        } catch (error) {
          const code = text(error?.code ?? error?.message) ?? 'fulfillment_task_failed';
          const retryable = Number(error?.status) >= 500
            && !/unknown_manual_reconciliation|order_already_shipped_unreconciled/u.test(code);
          logger.warn('fulfillment receipt task deferred', { taskId: task.id, error, retryable });
          await store.deferFulfillmentReceipt(task.id, task.lease, code, { retryable })
            .catch((deferError) => logger.warn('fulfillment receipt task defer failed', { taskId: task.id, error: deferError }));
        }
      }
    } catch (error) {
      logger.warn('fulfillment receipt poll deferred', { error });
    } finally {
  ...
  async function process(record) {
    ...
    try {
      ...
      if (ORDER_EVENT_TYPES.has(String(envelope.event ?? '').trim()) && typeof store.enqueueOrderEvent === 'function') {
        const outbound = orderEventPayload(envelope, session, backendOrder);
        if (!outbound.event_id || !outbound.tenant_id || !outbound.event_type) throw new Error('order_event_outbox_identity_invalid');
        await store.enqueueOrderEvent({
          tenant_id: outbound.tenant_id, event_id: outbound.event_id,
          event_type: outbound.event_type, idempotency_key: `order-event:v1:${outbound.event_id}`,  source_order_id: outbound.source_order_id,
          payload: outbound,
        });
        // Kick the delivery loop immediately; the interval remains as the
        // recovery path for events enqueued while the runtime is busy.
        void pollOrderEvents();
      }
  ...
  return Object.freeze({ start, stop, health, enqueue, enqueueFulfillmentStatus, syncTenantShops, listTenantOrders, getTenantOrder, fulfillTenantOrder, acceptFulfillmentReceipt, pollCommands, pollReminders, pollOrderEvents, pollFulfillmentReceipts, pollFulfillmentStatuses });
}
