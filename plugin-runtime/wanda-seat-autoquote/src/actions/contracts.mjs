...
export function normalizeAuthoritativeOrder(provider_order, { sdk_tenant_id } = {}) {
  ...
  const movie_name = pick(provider_order, ['movieName', 'movie_name', 'movie', 'filmName', 'film_name']);
  const cinema_name = pick(provider_order, ['cinemaName', 'cinema_name', 'cinema', 'theaterName', 'theater_name']);
  const showtime_start = pick(provider_order, ['showtimeStart', 'showtime_start', 'showtime', 'sessionStart', 'session_start']);
  const hall_name = pick(provider_order, ['hallName', 'hall_name', 'hall']);
  const seats = pick(provider_order, ['seatNames', 'seat_names', 'seats', 'seat']);
  const cost_amount = pick(provider_order, ['costAmountCents', 'cost_amount_cents', 'costAmountFen', 'cost_amount_fen'], { allow_null: true });
  const quote_amount = pick(provider_order, ['quoteAmountCents', 'quote_amount_cents', 'quoteAmountFen', 'quote_amount_fen'], { allow_null: true });
  const quote_record_id = pick(provider_order, ['quoteRecordId', 'quote_record_id'], { allow_null: true });
  const binding_revision = pick(provider_order, ['bindingRevision', 'binding_revision'], { allow_null: true });
  const selected = [order_id, provider_tenant_id, shop_id, buyer_id, chat_id, status, paid_at, amount, post_fee, refund_status, quantity, movie_name, cinema_name, showtime_start, hall_name, seats, cost_amount, quote_amount, quote_record_id, binding_revision];
  const tenant_id = text(provider_tenant_id.value) ?? text(sdk_tenant_id);
  const rawSeats = seats.value;
  const seat_names = Array.isArray(rawSeats)
    ? rawSeats.map((item) => text(item)).filter(Boolean)
    : typeof rawSeats === 'string' ? rawSeats.split(/[，,]/u).map((item) => text(item)).filter(Boolean) : null;
  ...
  return {
    ...
    movie_name: text(movie_name.value),
    cinema_name: text(cinema_name.value),
    showtime_start: text(showtime_start.value),
    hall_name: text(hall_name.value),
    seat_names,
    cost_amount_fen: integerCents(cost_amount.value),
    quote_amount_fen: integerCents(quote_amount.value),
    quote_record_id: text(quote_record_id.value),
    binding_revision: integerCents(binding_revision.value),
    field_sources: {
      ...
      movie_name: sourceEntry('provider_order', movie_name),
      cinema_name: sourceEntry('provider_order', cinema_name),
      showtime_start: sourceEntry('provider_order', showtime_start),
      hall_name: sourceEntry('provider_order', hall_name),
      seat_names: sourceEntry('provider_order', seats),
      cost_amount_fen: sourceEntry('provider_order', cost_amount),
      quote_amount_fen: sourceEntry('provider_order', quote_amount),
      quote_record_id: sourceEntry('provider_order', quote_record_id),
      binding_revision: sourceEntry('provider_order', binding_revision),
    },
