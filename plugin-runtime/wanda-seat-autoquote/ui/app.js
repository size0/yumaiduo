...
// Stable customer-service boundary for the future agent integration:
  ...
  function renderShops(shops) {
    ...
    shops.forEach(shop=>{
      ...
      const controls=element('div','shop-controls');const state=element('span',`shop-state${shop.enabled?'':' disabled'}`,shop.enabled?'自动回复与改价已开启':'自动化已关闭');const label=element('label','switch');const input=document.createElement('input');input.type='checkbox';input.checked=Boolean(shop.enabled);const slider=element('span','slider');label.append(input,slider);controls.append(state,label);row.append(copy,controls);root.append(row);input.addEventListener('change',()=>setShopEnabled(shop,input,state));
    });
  }
  ...
  function renderQuoteRecords(records) {
    const root=$('quoteRecordList');root.replaceChildren();const failed=records.filter(record=>record.status==='failed').length;$('quoteRecordSummary').textContent=`共 ${records.length} 条核价记录 · 成功 ${records.length-failed} 条 · 失败 ${failed} 条`;
    if(!records.length){root.append(element('div','diagnostic-empty','暂无核价记录。新产生的官方报价及失败原因会自动显示在这里。'));return;}
    records.forEach(record=>{const card=element('article','record-card');const primary=element('div','record-primary');primary.append(element('strong','',`${record.city?`${record.city} · `:''}${record.cinema||'影院待确认'}`),element('small','',`${record.movie||'影片待确认'} · ${record.quote_date||record.date_text||'日期待确认'} ${record.showtime_start||''}\n${record.seat_display||record.seat_zone_type||'座位待确认'}`));if(record.status==='failed'){const status=recordFact('核价状态','失败');status.querySelector('b').classList.add('record-status','record-status-failed');card.append(primary,status,recordFact('失败原因',record.failure_reason||'未取得权威报价','wide'),recordFact('买家 / 时间',`${record.buyer_id||'—'} · ${formatRecordTime(record.created_at)}`));}else{card.append(primary,recordFact('报价单价',centsAmount(record.unit_quote_cents)),recordFact('报价合计',centsAmount(record.total_quote_cents)),recordFact('买家 / 时间',`${record.buyer_id||'—'} · ${formatRecordTime(record.created_at)}`));}root.append(card);});
  }
  async function loadQuoteRecords() {
    $('quoteRecordList').replaceChildren(element('div','diagnostic-empty','正在读取报价记录…'));
    try{const response=await v4Fetch('/api/plugin/quote-records?limit=200');const data=await response.json();if(!response.ok)throw new Error(data?.detail||'读取报价记录失败');renderQuoteRecords(data.records||[]);}catch(error){$('quoteRecordSummary').textContent='读取失败';$('quoteRecordList').replaceChildren(element('div','diagnostic-empty',error.message));}
  }
  let loadedOrders=[];let linkedOrderFacts=new Map();let observedOrderCount=0;let activeOrderFilter='all';
  function orderCategory(order){const status=String(order.orderStatus??'').toLowerCase();const label=String(order.orderStatusText||'').toLowerCase();if(status==='1'||/待付款|等待付款|pending|wait_pay/.test(label))return'unpaid';if(status==='2'||/已付款|支付成功|paid/.test(label))return'paid';if(status==='3'||/已发货|待收货|shipped/.test(label))return'shipped';if(['4','5','60'].includes(status)||/完成|关闭|取消|退款|completed|closed|cancel|refund/.test(label))return'closed';return'other';}
  ...
  function orderShowtime(fact){if(!fact?.quote_date&&!fact?.showtime_start)return'开场时间待关联';return`${fact.quote_date||'日期待确认'}${fact.showtime_start?` ${fact.showtime_start}`:''}`;}
  function orderMatches(order,fact,query){if(!query)return true;return[order.orderId,order.buyerNick,order.productTitle,order.sku,fact?.city,fact?.cinema,fact?.movie,fact?.hall,fact?.seat_display].some(value=>String(value||'').toLowerCase().includes(query));}
  function orderStatusFact(order){const status=recordFact('订单状态',order.orderStatusText||String(order.orderStatus??'未知'));status.querySelector('b').classList.add('record-status',`order-status-${orderCategory(order)}`);return status;}
  function renderOrders(){
    const query=String($('orderSearch').value||'').trim().toLowerCase();const visible=loadedOrders.filter(order=>{const category=orderCategory(order);const fact=linkedOrderFacts.get(String(order.orderId));return(activeOrderFilter==='all'||activeOrderFilter===category)&&orderMatches(order,fact,query)});const root=$('orderList');root.replaceChildren();const linked=loadedOrders.filter(order=>linkedOrderFacts.has(String(order.orderId))).length;$('orderSummary').textContent=`已回读 ${loadedOrders.length} 笔订单 · ${linked} 笔已关联观影信息 · 当前显示 ${visible.length} 笔${observedOrderCount>loadedOrders.length?` · ${observedOrderCount-loadedOrders.length} 笔暂不可用`:''}`;
    if(!visible.length){root.append(element('div','diagnostic-empty',loadedOrders.length?'没有符合当前筛选条件的订单。':'暂无可读取订单。插件收到订单事件后会自动显示在这里。'));return;}
    visible.forEach(order=>{const fact=linkedOrderFacts.get(String(order.orderId));const card=element('article','order-card');const main=element('div','order-card-main');const eyebrow=element('div','order-eyebrow');eyebrow.append(element('span','order-city',fact?.city||'影院信息待关联'),element('span','order-number',`订单 ${order.orderId}`));main.append(eyebrow,element('strong','order-cinema',fact?.cinema||order.productTitle||order.sku||'影院信息待关联'),element('h4','order-movie',fact?.movie||'影片信息待关联'));const schedule=element('div','order-schedule');schedule.append(element('span','',`◷ ${orderShowtime(fact)}`),element('span','',`▣ ${fact?.hall||'影厅待确认'}`));main.append(schedule,element('div','order-seats',fact?.seat_display||fact?.seat_zone_type||'座位信息待关联'),element('small','order-meta',`店铺 ${order.accountUnb||'—'} · 下单 ${formatRecordTime(order.createTime||order.observedAt)}`));const side=element('div','order-card-side');side.append(orderStatusFact(order),recordFact('订单金额',centsAmount(order.payment)),recordFact('商品数量 / 买家',`${order.quantity??'—'} · ${order.buyerNick||'—'}`),recordFact('开场时间',orderShowtime(fact),'order-reminder-fact'));card.append(main,side);root.append(card);});
  }
  async function loadOrders() {
    $('orderList').replaceChildren(element('div','diagnostic-empty','正在回读订单并关联影院信息…'));
    try{await fishMoreReady;const[orderResponse,factResponse]=await Promise.all([fishMoreSdk.authedFetch('ui/api/orders?limit=100'),v4Fetch('/api/plugin/quote-records?limit=500')]);const data=await orderResponse.json();const facts=await factResponse.json();if(!orderResponse.ok)throw new Error(data?.error||'读取订单失败');if(!factResponse.ok)throw new Error(facts?.detail||'读取订单观影信息失败');loadedOrders=data.orders||[];observedOrderCount=Number(data.observedCount)||0;linkedOrderFacts=linkedFactMap(facts.records||[]);renderOrders();}catch(error){$('orderSummary').textContent='读取失败';$('orderList').replaceChildren(element('div','diagnostic-empty',error.message));}
  }
  ...
  let preservedRegularAdjustmentCents=100;
  ...
  async function loadOperations() {
    try {
      ...
      $('pricingEnabled').checked=data.enabled; $('wplusDiscount').value=(Math.abs(data.wplus_adjustment_cents)/100).toFixed(2); $('wplusThreshold').value=(data.wplus_member_price_threshold_cents/100).toFixed(2); $('roundingIncrement').value=String(data.rounding_increment_cents);
  ...
  async function saveOperations() {
    ...
    try {
      const payload={enabled:$('pricingEnabled').checked,regular_adjustment_cents:preservedRegularAdjustmentCents,wplus_adjustment_cents:-yuanToCents('wplusDiscount'),wplus_member_price_threshold_cents:yuanToCents('wplusThreshold'),rounding_increment_cents:Number($('roundingIncrement').value)};
  ...
  $('refreshDiagnostics').addEventListener('click',refreshDiagnostics); $('clearDiagnostics').addEventListener('click',clearDiagnostics); $('refreshShops').addEventListener('click',()=>loadShops(true)); $('refreshQuoteRecords').addEventListener('click',loadQuoteRecords); $('refreshOrders').addEventListener('click',()=>{loadOrders();loadReminderTasks();}); $('saveReminders').addEventListener('click',saveReminderSettings); $('reminderEnabled').addEventListener('change',()=>{$('reminderEnabledText').textContent=$('reminderEnabled').checked?'已开启':'已关闭'}); $('orderSearch').addEventListener('input',renderOrders);
  ...
  showWorkspace(location.hash.replace('#','')||'chat'); updateSend(); initLiangpiaoRules(); loadSettings(); loadOperations(); loadConversationPolicy(); loadKnowledge(); refreshDiagnostics(); setInterval(()=>{if(!document.hidden&&activeWorkspace==='logs')refreshDiagnostics();},4000);
