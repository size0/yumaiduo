...
// Stable customer-service boundary for the future agent integration:
  ...
  function wandaOrderRow(order,fact){return{source:'wanda',detailId:String(order.orderId||''),orderId:String(order.orderId||''),movie:fact?.movie||order.productTitle||order.sku||'—',city:fact?.city||'—',cinema:fact?.cinema||'—',showtime:orderShowtime({showtime:fact?`${fact.quote_date||fact.date_text||''}${fact.showtime_start?` ${fact.showtime_start}`:''}`.trim():''}),seats:fact?.seat_display||fact?.seat_zone_type||'—',ticketMode:'手动出票',marketAmountFen:fact?.base_total_cents,quoteAmountFen:fact?.total_quote_cents,dealAmountFen:order.payment,status:order.orderStatusText||String(order.orderStatus??'未知'),statusText:order.orderStatusText||String(order.orderStatus??'未知'),createdAt:order.createTime||order.observedAt,buyerNick:order.buyerNick||'—',accountUnb:order.accountUnb,raw:order,fact};}
