...
// Stable customer-service boundary for the future agent integration:
  ...
  showWorkspace(location.hash.replace('#','')||'chat'); updateSend(); initLiangpiaoRules(); initWandaRules(); loadSettings(); loadOperations(); loadConversationPolicy(); loadKnowledge(); refreshDiagnostics(); setInterval(()=>{if(!document.hidden&&activeWorkspace==='logs')refreshDiagnostics();},4000); setInterval(()=>{if(!document.hidden&&activeWorkspace==='orders')void loadOrders();},15000);
