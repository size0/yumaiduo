...
// Stable customer-service boundary for the future agent integration:
  ...
  function overviewValue(label,value) { const row=element('div','config-value');row.append(element('span','',label),element('b','',value||'—'));return row; }
  ...
  function renderQuoteRecordDetail(record) {
    const root=$('quoteRecordDetail');root.replaceChildren();const heading=element('div','detail-panel-head');heading.append(element('span','eyebrow','QUOTE DETAIL'),element('h3','',record.cinema||'报价详情'),element('small','',`${record.movie||'影片待确认'} · ${formatRecordTime(record.created_at)}`));root.append(heading);
    const facts=element('div','detail-fact-list');[['核价状态',record.status==='failed'?'失败':'成功'],['买家',record.buyer_id],['城市',record.city],['影院',record.cinema],['场次',`${record.quote_date||record.date_text||'—'} ${record.showtime_start||''}`.trim()],['座位',record.seat_display||record.seat_zone_type],['原价',centsAmount(record.original_unit_price_cents)],['会员价',centsAmount(record.member_unit_price_cents)],['报价单价',centsAmount(record.unit_quote_cents)],['报价合计',centsAmount(record.total_quote_cents)],['规则版本',record.pricing_rule_version],['失败原因',record.failure_reason]].forEach(([label,value])=>facts.append(overviewValue(label,value)));root.append(facts);
  }
  function renderQuoteRecords(records) {
    ...
    if(!records.length){root.append(element('div','diagnostic-empty','暂无核价记录。新产生的官方报价及失败原因会自动显示在这里。'));$('quoteRecordDetail').replaceChildren(element('div','detail-panel-empty','暂无可展示报价详情'));return;}
    records.forEach((record,index)=>{const card=element('article',`record-card${index===0?' selected':''}`);card.tabIndex=0;const primary=element('div','record-primary');primary.append(element('strong','',`${record.city?`${record.city} · `:''}${record.cinema||'影院待确认'}`),element('small','',`${record.movie||'影片待确认'} · ${record.quote_date||record.date_text||'日期待确认'} ${record.showtime_start||''}\n${record.seat_display||record.seat_zone_type||'座位待确认'}`));if(record.status==='failed'){const status=recordFact('核价状态','失败');status.querySelector('b').classList.add('record-status','record-status-failed');card.append(primary,status,recordFact('失败原因',record.failure_reason||'未取得权威报价','wide'),recordFact('买家 / 时间',`${record.buyer_id||'—'} · ${formatRecordTime(record.created_at)}`));}else{card.append(primary,recordFact('原价',centsAmount(record.original_unit_price_cents)),recordFact('会员价',centsAmount(record.member_unit_price_cents)),recordFact('报价单价',centsAmount(record.unit_quote_cents)),recordFact('报价合计',centsAmount(record.total_quote_cents)),recordFact('买家 / 时间',`${record.buyer_id||'—'} · ${formatRecordTime(record.created_at)}`));}const select=()=>{root.querySelectorAll('.record-card').forEach(node=>node.classList.toggle('selected',node===card));renderQuoteRecordDetail(record);};card.addEventListener('click',select);card.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();select();}});root.append(card);});renderQuoteRecordDetail(records[0]);
  }
  ...
  function createWandaRuleRow({min=0,max=0,adjustment=0}={}) {
    ...
    const fixed=document.createElement('input'); fixed.className='wanda-adjustment'; fixed.type='number'; fixed.min='-1000'; fixed.max='1000'; fixed.step='0.1'; fixed.value=String(adjustment); fixed.setAttribute('aria-label','固定加价/减价');
